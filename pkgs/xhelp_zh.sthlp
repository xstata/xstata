...
{cmd:xhelp} [{it:命令或主题名}] [{cmd:,}
  ...
  {opt install} {opt install(帮助文档压缩包路径)} 
...
{pstd}
{cmd:xhelp}命令提供{bf:安装}、{bf:创建}和{bf:展示} Stata 翻译版帮助文档的工具，
主要有三种工作模式：
...
{pstd}
在 {it:安装模式} 中，{cmd:xhelp} 将指定位置的预打包的翻译版帮助文档解压缩至命名为
PLUS的ADO文档路径下。
...
{p 4 8 2}{opt install} 启动 {it:安装模式}，此时 {cmd:xhelp} 将 Stata 当前工作目录下的
 xhelp_sthlps.zip 翻译版帮助文档压缩包解压缩至命名为PLUS的ADO文档路径下。

{p 4 8 2}{opt install(帮助文档压缩包路径)} 同样启动 {it:安装模式}，但此时你可以指定翻译版帮助文档压缩包
所在的具体路径（含压缩包文件名）。
...
{p 4 7 2}{ul:2. {it:安装模式}}:{p_end}

{p 8 10 2}
- 你通过设定选项 {opt install} 或 {opt install(帮助文档压缩包路径)} 来进入 {it:安装模式}。
{p_end}
...
{phang}{cmd:. xhelp, install}{space 9} // 安装模式，请勿运行 {p_end}
{phang}{cmd:. xhelp, install(C:\Users\xxx\Desktop\Stata-xhelp\xhelp_sthlps.zip)}{space 2} // 安装模式，请勿运行 {p_end}
